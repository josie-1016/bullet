...
//	//fmt.Println(nprime)
//	//fmt.Println(len(H))
//	cl := InnerProduct(a[:nprime], b[nprime:], ec.N) // either this line  <ahi, blo>
//	cr := InnerProduct(a[nprime:], b[:nprime], ec.N) // or this line  <alo, bhi>
...
//
//	ccalc := new(big.Int).Mod(new(big.Int).Mul(ipp.A, ipp.B), ec.N)
//	lhs := TwoVectorPCommitWithGens(G, H, ScalarVectorMul(sScalars, ipp.A, ec.N), ScalarVectorMul(invsScalars, ipp.B, ec.N), ec).Add(ux.Mult(ccalc, ec), ec)
//
//	if !rhs.Equal(lhs) {
//		fmt.Println("IPVerify - Final Commitment checking failed")
